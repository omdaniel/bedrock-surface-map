...
if (!target || !outputArg)
  throw Error("usage: build-native.mjs <target> <output>");
...
for (const packageName of packages) {
  if (!binaries.has(packageName))
    throw Error(`Cargo metadata has no binary for ${packageName}`);
}
execFileSync(
  "cargo",
  [
...
for (const packageName of packages) {
  ...
  await cp(
    resolve("target", target, "release", binary),
    resolve(output, binary),
  );
}
